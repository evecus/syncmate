...
/// 设备长连接事件（入站连接建立 / 断开）。
sealed class ConnectionServerEvent {}

class ConnectionInbound extends ConnectionServerEvent {
  ConnectionInbound(this.fingerprint);
  final String fingerprint;
}

class ConnectionInboundClosed extends ConnectionServerEvent {
  ConnectionInboundClosed(this.fingerprint);
  final String fingerprint;
}
...
class SyncMateServer {
  ...
  final Map<String, WebSocket> _activeConnectionSockets = {};
  final StreamController<ConnectionServerEvent> _connectionEvents =
      StreamController<ConnectionServerEvent>.broadcast();
  ...
  /// 设备长连接事件（入站连接建立 / 断开）。
  Stream<ConnectionServerEvent> get connectionEvents => _connectionEvents.stream;

  /// 当前有入站长连接的设备指纹集合（对方主动连过来的）。
  Set<String> get inboundConnectedFingerprints =>
      Set.unmodifiable(_activeConnectionSockets.keys);
  ...
  Future<void> stop() async {
    ...
    for (final socket in _activeConnectionSockets.values) {
      try {
        await socket.close();
      } on Object {
        // ignore
    ...
    _activeConnectionSockets.clear();
  ...
  Future<void> _handle(HttpRequest request) async {
    try {
      ...
      } else if (path == Constants.connectionWsPath && request.method == 'GET') {
        await _handleConnectionWs(request);
      } else {
        await _writeError(
          request,
          HttpStatus.notFound,
  ...
  /// 设备长连接 WS 端点：校验信任头后升级；同一来源指纹的新连接顶掉旧连接。
  /// 仅维护存活状态与心跳，不承载业务消息（文件操作仍走普通 HTTP 接口）。
  Future<void> _handleConnectionWs(HttpRequest request) async {
    final fingerprint = request.headers.value(_headerFingerprint);
    if (fingerprint == null ||
        !Constants.fingerprintPattern.hasMatch(fingerprint) ||
        await _trustStore.find(fingerprint) == null) {
    ...
    final previous = _activeConnectionSockets[fingerprint];
    if (previous != null) {
      try {
        await previous.close();
      } on Object {
    ...
    _activeConnectionSockets[fingerprint] = ws;
    _connectionEvents.add(ConnectionInbound(fingerprint));
    ws.listen(
      (data) {
        if (data is! String) return;
        if (data == '{"type":"ping"}') {
      ...
      onDone: () {
        if (identical(_activeConnectionSockets[fingerprint], ws)) {
          _activeConnectionSockets.remove(fingerprint);
          _connectionEvents.add(ConnectionInboundClosed(fingerprint));
        }
      },
      onError: (Object _) {
        if (identical(_activeConnectionSockets[fingerprint], ws)) {
